...
export default async function VerifyPage(props: { params: Promise<{ token: string }> }) {
  ...
  const user = wh
    ? (await db.select().from(schema.users).where(eq(schema.users.id, wh.userId)).limit(1))[0]
    : null;
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-6 py-16">
      ...
      <p className="mt-3 text-[14px] text-[var(--muted)]">
        ...
        to confirm it. Confirming ties a cryptographic signature to this entry; denying flags it as
        disputed. Neither action reveals your identity to the public profile.
      </p>
        ...
        <CardBody>
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              {user?.githubUsername && (
                <div className="num text-[12px] text-[var(--muted)]">@{user.githubUsername}</div>
              )}
            </div>
            <Badge
              tone={
                status === 'confirmed' ? 'verified' : status === 'pending' ? 'outline' : 'neutral'
              }
            >
              {status}
            </Badge>
